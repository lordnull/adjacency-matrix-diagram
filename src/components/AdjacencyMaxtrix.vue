                    ...
                    <svg :width="`${(length + rooms.length + 20) * baseLengthUnit * 1.5 }`" :height="`${(rooms.length + 1) * baseLengthUnit * 3}`" id="svg-image">
                        ...
                        >
                            <rect :x="r.rect.x" :y="r.rect.y" :width="r.rect.width" :height="r.rect.height"
                                ...
                                class="rect"
                            ></rect>
                            <StatePrimary v-if="r.state === 'Primary'" @click="changeState({ r, ri })"/>
                            <StateSecondary v-if="r.state === 'Secondary'" @click="changeState({ r, ri })"/>
    ...
    export default {
        ...
        methods: {
            ...
            changeState({ r }) {
                ...
                this.$set(this.states, key, state);
                this.saveState();
            },
                ...
                if (rawState) {
                    try {
                        ...
                        Object.keys(savedState.states).forEach((k) => {
                            this.$set(this.states, k, savedState.states[k]);
                        })
                    } catch(err) {
